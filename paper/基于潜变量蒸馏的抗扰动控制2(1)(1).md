...
| :--- | :---: | :---: | :---: | :--- |
| **Ours (Full)** | **100%** | **0.0815 m** | **2.6°** | 完整方案: 残差架构 + 潜变量蒸馏 + 物理随机化 |
| w/o Physics Randomization | 100% | 0.2765 m (+239%) | 4.1° (+58%) | 关闭电机/阻力/外力随机化 |
| Pure PD (Baseline) | 92% | 0.3600 m (+342%) | 7.7° (+196%) | 纯PD控制器，无RL补偿 |

*测试条件: Hard 难度 (风力 $[0, 0.2]$ N, 载荷 $[0, 0.03]$ kg), 每配置 50 episodes*

*坠机判定: 当姿态倾角超过 $\theta_{max}=60°$ 或位置误差超过 $d_{max}=5m$ 时，判定为失控坠机*

> [!NOTE] **消融实验发现**
> 1. **RL 补偿必要性**: 纯 PD 控制器在载荷释放后出现 **8% 坠机率**，而完整方案保持 **100% 存活**
> 2. **物理随机化价值**: 位置精度提升 **3.4 倍** (RMSE: 0.0815m vs 0.2765m)，姿态稳定性提升 **58%**
> 3. **综合优势**: 相比 baseline，位置精度提升 **4.4 倍**，倾角抖动减少 **66%**
